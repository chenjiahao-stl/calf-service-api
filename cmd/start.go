...
var StartCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		// 加载程序配置
		err := conf.LoadTomlConfig(confFile)
		if err != nil {
			fmt.Errorf("load toml config error:%s", err)
			return err
		}
		// 初始化全局日志Logger
		if err := loadGlobalLogger(); err != nil {
			fmt.Errorf("load Global Logger error:%s", err)
			return err
		}
		...
		//    main { for range channel }  这个时候的channel仅仅相当于一个缓存, 必须选择带缓存区的channl
		...
		signal.Notify(ch, syscall.SIGTERM, syscall.SIGQUIT, syscall.SIGHUP, syscall.SIGINT) //处理操作系统发过来的信号
...
func init() {
	StartCmd.PersistentFlags().StringVarP(&confFile, "config", "f", "etc/demo.toml", "restful api 配置文件路径")
